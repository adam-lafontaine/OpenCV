...
constexpr auto HEAT_MAP_PATH = "D:/repos/OpenCV/Tutorial/out_files/heat_map.jpg";
constexpr auto FLOODED_PATH = "D:/repos/OpenCV/Tutorial/out_files/flooded.jpg";
constexpr auto SRTM_PATH = "D:/repos/OpenCV/Tutorial/in_files/N37W123.hgt";
constexpr auto SRTM_CODE = "SRTM";
...
//    Note that GDAL library can natively determine this
    ...
    */
    ...
    {        
        ...
        cv::Mat image = cv::imread(SRTM_PATH, cv::IMREAD_LOAD_GDAL | cv::IMREAD_COLOR);

        // load the dem model
        cv::Mat dem = cv::imread(SRTM_CODE, cv::IMREAD_LOAD_GDAL | cv::IMREAD_ANYDEPTH);

        if (image.empty() || dem.empty())
        {
        ...
        for (int y = 0; y < image.rows; y++) 
        {
            for (int x = 0; x < image.cols; x++) 
            {
                ...
                double dz;
                if (dem_coordinate.x >= 0 
                    && dem_coordinate.y >= 0 
                    && dem_coordinate.x < dem.cols 
                    && dem_coordinate.y < dem.rows) 
                {
        ...
        // print our heat map
        cv::imwrite(HEAT_MAP_PATH, output_dem);

        // print the flooding effect image
        cv::imwrite(FLOODED_PATH, output_dem_flood);
    }
    ...
    */
    ...
    cv::Vec<DATATYPE, N> lerp(
        cv::Vec<DATATYPE, N> const& minColor,
